...
normalize ( s, e ) =
    if s == 0 || e == 0 || remainderBy 10 s /= 0 then
        ( s, e )
...
toString ( s, e ) =
    if e == 0 then
        ...
        let
            numeric_string =
                s |> String.fromInt |> String.padLeft -e '0'
        in
        let
            characteristic =
                numeric_string |> String.slice 0 e
        in
        let
            mantissa =
                numeric_string |> String.right -e
        in
...
add ( xs, xe ) ( ys, ye ) =
    ...
    in
    let
        normalizedWithExponent =
            exponent |> withExponent
    in
    let
        ( a, _ ) =
            normalizedWithExponent ( xs, xe )
    in
    let
        ( b, _ ) =
            normalizedWithExponent ( ys, ye )
    in
...
divide ( xs, xe ) ( ys, ye ) =
    (toFloat xs * (10.0 ^ toFloat xe))
        / (Basics.toFloat ys * (10.0 ^ toFloat ye))
        |> toNumber
        |> normalize
